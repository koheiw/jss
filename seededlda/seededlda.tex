...
\caption{Graphical model of a simple LDA. Gray circles are variables
whose values are known, whereas white circles are latent variables whose
values are unknown. \(\theta\) and \(\phi\) are the \(|D| \times K\) and
\(K \times |V|\) matrices, respectively; \(Z\) is a vector to record
topics words, \(W\), in document \(d\); \(d\) is one of the documents
\(d \in D\); \(v\) is one of the unique words \(v \in V\) in the
corpus.}
\end{figure}
